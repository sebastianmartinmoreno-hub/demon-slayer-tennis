...
import {
  Brain, Flame, Flower2, Lightning, ListTodo, Medal,
  Play, Pause, RefreshCcw, Settings, Shield, Swords, Waves, Wind
} from "lucide-react";
...
const FORMS = {
  ...
  water: {
    ...
    pattern: [{ phase: "Inhale", sec: 4 }, { phase: "Exhale", sec: 6 }],
    cycles: 8,
  },
  thunder: {
    key: "thunder",
    ...
    pattern: [{ phase: "Inhale", sec: 2 }, { phase: "Exhale", sec: 2 }],
    cycles: 15,
  },
  wind: {
    key: "wind",
    ...
    pattern: [{ phase: "Inhale", sec: 4 }, { phase: "Exhale", sec: 8 }],
    cycles: 10,
  },
  flower: {
    key: "flower",
    ...
    pattern: [{ phase: "Inhale", sec: 4 }, { phase: "Hold", sec: 7 }, { phase: "Exhale", sec: 8 }],
    cycles: 4,
  },
  flame: {
    key: "flame",
  ...
  return (
    <header className="sticky top-0 z-20 backdrop-blur bg-black/30 border-b border-white/10">
      <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
        <span className="text-xl font-extrabold tracking-wide bg-gradient-to-r from-red-400 to-amber-300 bg-clip-text text-transparent">
          Demon Slayer Tennis
        </span>
        <nav className="ml-auto flex gap-1">
          {tabs.map(t => (
            <button key={t.id} onClick={() => setPage(t.id)}
              className={cx("px-3 py-1.5 rounded-xl text-sm flex items-center gap-2",
                            page===t.id ? "bg-white/10" : "hover:bg-white/5")}>
      ...
      {ACHIEVEMENTS.map(a => (
        <div key={a.id}
             className={cx("rounded-xl p-3 border",
                           unlocked.find(u => u.id === a.id) ? "border-emerald-400/40 bg-emerald-400/10"
...
function TrainPage({ formKey, setFormKey, onComplete, addSession, xp, stats }) {
  ...
  const timer = useBreathTimer(form);

  React.useEffect(() => {
    const done = !timer.running && timer.cycle === form.cycles &&
                 timer.stepIndex === form.pattern.length - 1 && timer.secLeft === 1;
    if (done) {
    ...
    <div className="grid md:grid-cols-3 gap-4">
      <div className="md:col-span-1 space-y-2">
        {Object.values(FORMS).map(f => (
          <FormCard key={f.key} form={f} active={f.key===formKey} onSelect={() => setFormKey(f.key)} />
        ))}
        <XPBar xp={xp} />
        <div className="rounded-2xl p-4 border border-white/10 bg-white/5">
